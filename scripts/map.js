// Initialize project data and map
let allProjects = [];

// Helper: calculate time since planned date
function timeSince(plannedDateStr) {
...
function getMarkerIcon(status) {
    return L.icon({
        iconUrl: status === "In-Progress" ? "assets/marker-icon-green.png" : "assets/marker-icon-blue.png",
        iconSize: [25, 41],
        iconAnchor: [12, 41],
        popupAnchor: [0, -40],
...
// Main function: render/filter markers
function updateMarkers() {
    markerGroup.clearLayers();
    const province = document.getElementById("province-filter").value;
    const minBudget = parseFloat(document.getElementById("budget-filter").value) * 1e9 || 0;
    allProjects
        .filter(p =>
...
// Load project data and setup
fetch('https://backend-api-production-e1a3.up.railway.app/api/projects')
    .then(res => res.json())
    .then(data => {
        allProjects = data;
        updateMarkers();
